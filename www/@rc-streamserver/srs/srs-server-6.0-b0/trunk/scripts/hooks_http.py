...
def _hooks_debug(msg: str) -> None:
    """기본 비활성. 상세 로그: SRS_HOOKS_VERBOSE=1"""
    if os.environ.get("SRS_HOOKS_VERBOSE", "") == "1":
...
class HookHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        """기본 HTTP 액세스 로그 비활성화(journal 스팸 방지). SRS_HOOKS_VERBOSE=1 일 때만."""
        if os.environ.get("SRS_HOOKS_VERBOSE", "") == "1":
            ...
            for c in clients:
                try:
                    ...
                    if should_kill:
                        ...
                        _hooks_debug(
                            f"[HOOKS] takeover: kill old publisher stream={stream_name} old_cid={old_cid} old_ip={c_ip} alive={alive:.1f}s recv_30s={recv_30s} ok={ok}"
                        )
                except Exception:
                    continue
        except Exception:
    ...
    def do_POST(self):
        ...
        if self.path in ('/on_publish', '/on_unpublish'):
            if self.path == '/on_publish':
                ...
                if stream_name:
                    _hooks_debug(
                        f"[HOOKS] on_publish: client_id={client_id}, stream={stream_name}, app={app_name}, ip={ip_addr}"
                    )
                ...
                if stream_name:
                    _hooks_debug(
                        f"[HOOKS] on_unpublish: client_id={client_id}, stream={stream_name}, app={app_name}"
                    )
...
def main():
    ...
    if HookHandler.ENABLE_WATCHDOG_KILL:
        ...
        _hooks_debug("[HOOKS] watchdog_kill=enabled")
    else:
        _hooks_debug("[HOOKS] watchdog_kill=disabled")

    httpd = ThreadingHTTPServer((host, port), HookHandler)
    _hooks_debug(f"srs hooks http listening on {host}:{port}")
    httpd.serve_forever()
